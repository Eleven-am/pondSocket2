...
import {ChannelRequest} from "../channel/channelRequest";
import {ChannelResponse} from "../channel/channelResponse";
import {WebSocket} from "ws";
import {PondChannelResponse} from "./pondChannelResponse";
import {PondPath} from "../utils/matchPattern";
import {PondChannelRequest} from "./pondChannelRequest";

export type AuthorizeMiddleware = (request: PondChannelRequest, response: PondChannelResponse) => void | Promise<void>;
...
export class PondChannel {
    ...
    private readonly _middleware: Middleware<ChannelRequest, ChannelResponse>;
     ...
     * });
     */
    public onEvent(event: PondPath, handler: (request: ChannelRequest, response: ChannelResponse) => void | Promise<void>) {
    ...
    private _addUser(user: RequestCache) {
        ...
        if (this._authorize) {
            ...
            const request = new PondChannelRequest(user, newChannel);
            const response = new PondChannelResponse(socketCache, newChannel);
